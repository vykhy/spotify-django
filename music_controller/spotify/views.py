...
class  CurrentSong(APIView):
    def get(self, request, format=None):
        ...
        # number of votes given to skip current song
...
class SkipSong(APIView):
    def post(self, request, format=None):
        # get room, number of votes needed, and number of votes
        room_code = self.request.session.get('room_code')
        room = Room.objects.filter(code=room_code)[0]
        votes = Vote.objects.filter(room=room, song_id=room.current_song)
        votes_needed = room.votes_to_skip

        # if host skips, or if votes meet minimum number to skip, skip 
        if self.request.session.session_key == room.host or len(votes) + 1 >= votes_needed:
            votes.delete()
            skip_song(room.host)
        # add a vote to skip the song
        else:
            # existing_vote = Vote.objects.filter(room=room, song_id=room.current_song, user=self.request.session.session_key)
